...
class AiTradingAgent
{
    ...
    {
        try {
            return $this->technicals->gatherMarketData($symbol);
        } catch (\Exception $e) {
    ...
    private function buildAnalysisPrompt(array $context): string
    {
        ...
        $gridPos = $b['price_upper'] > $b['price_lower']
            ? round(($m['price'] - $b['price_lower']) / ($b['price_upper'] - $b['price_lower']) * 100, 1)
            : 50;

        return <<<PROMPT
{$b['symbol']} Price:{$m['price']} 24h:{$m['chg24h']}% RSI:{$m['rsi']} MACD:{$m['macd']} Vol:{$m['vol_ratio']}x ATR:{$m['atr']} BB:{$m['bb_lower']}/{$m['bb_upper']}
